...
def train(
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
         ...
         # forward pass
        if isinstance(model, transformers.ViTForImageClassification):  # Check if model is ViT
            output = model(pixel_values=data)
...
def validation(
    ...
    for data, target in val_loader:
        ...
        # sum up batch loss
        criterion = torch.nn.CrossEntropyLoss(reduction="mean")
        validation_loss += criterion(logits, target).data.item()
...
def main():
    ...
    # Setup optimizer
    if args.model_name == "dinov2_vits14" or args.model_name == "dinov2_vitb14_reg"  or args.model_name == "dinov2_vitb14":
        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
    elif args.model_name == 'vit':
        # optimizer = torch.optim.AdamW(model.classifier.parameters(), lr=args.lr,weight_decay=0.001)
         optimizer = torch.optim.AdamW(model.classifier.parameters(), lr=args.lr)
    else:
        # for basic_cnn and extended_cnn
...
# python main.py --seed 42 --experiment "experiment_1" --model_name "basic_cnn" --data "data" --batch_size 32 --num_workers 4 --lr 0.01 --momentum 0.9 --epochs 1
